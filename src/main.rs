...
fn moving_average(data: &[f64], window_size: usize) -> Vec<f64> {
  ...
  fn get_max_f64(values: &[f64]) -> f64 {
    ...
    fn process_flight_log(file_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let smoothed_speeds = moving_average(&speeds, window_size);
            ...
            let output_path = format!("flight_data_plot_rust_{}.png", output_name);
        let root = BitMapBackend::new(&output_path, (1200, 1200))
            .into_drawing_area();
        root.fill(&WHITE)?;
    ...
    fn main() -> Result<(), Box<dyn std::error::Error>> {
        // Add walkdir dependency to Cargo.toml first
        for entry in WalkDir::new(".")
            .follow_links(true)
            .into_iter()
            .filter_map(|e| e.ok()) {
            
            if entry.file_name() == "flight_log.json" {
                println!("Processing: {}", entry.path().display());
                process_flight_log(entry.path())?;
            }
        }
        Ok(())
    }
